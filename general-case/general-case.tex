...
Revenue management problems can be often formulated as optimization problems
where the variables encode the actions that need to be taken. However, often the
actions need to be taken one by one, before the whole optimization problem
is revealed.
...
a constraints on different business metrics (e.g. total advertisement cost, or
...
\end{align}
\end{subequations}
where $b_1, b_2, \dots, b_M$ are real numbers known ahead of time. A sequence of
actions taken by the algorithm is an approximate solution of this problem if
constraints \eqref{equation:optimization-problem-hard-constraints} are
satisfied, constraints \eqref{equation:optimization-problem-constraints} are
violated by at most a small value $\epsilon$, and the objective
\eqref{equation:optimization-problem-objective} is within tolerance $\epsilon$
of the optimal value; see Section~\ref{subsection:approximate-solutions}.

The formalization does not fully capture the applications mentioned above.
First, in the applications mentioned above, the elements of $R_t$ are predicted
by machine learning models just before the action $a_t$ needs to be chosen.
These predictions are only proxies for the business metrics. The actual business
metrics are revealed only after the action has been chosen, sometimes with a
significant delay. However, in order to keep our problem formulation simple, we
ignore the difference between the predicted and the actual business metrics.
Second, in the applications mentioned, the constraints are usually expressed as
upper bounds on the total sum,
$$
...
simple, we assume that $b_i$ is known ahead of time. As it will turn out knowing
$T$ ahead of time is not neccesary in our formulation. However, for simplicity
we do assume that $T$ is known. Adapting algorithms to unknown $T$ can be
achieved by the standard doubling trick common on online learning~\cite[Section
...
approximately infeasible) and either removes or adjusts some of the
constraints, or stops playing.

Without any assumptions on the sequence of requests solving the online
optimization problem is information-theoretically impossible. In order to avoid
this impossibility result, we assume that sequence of requests is generated
i.i.d. from an arbitrary unknown distribution. This is similar to the probably
approximately correct (PAC) model in learning theory~\cite[Chapter
3]{Shalev-Shwartz-Ben-David-2014}. The i.i.d. assumption is not new; packing
problems were studied under this assumption~\cite{Balseiro-Lu-Mirrokni-2023}. On
the other hand, we make very few assumptions on the sets $R_1, R_2, \dots, R_T$.
We do allow each $R_t$ to be finite or infinite, convex or non-convex. The only
