...
\begin{aligned}
\maximize_{x_1, x_2, \dots, x_T} \qquad & \frac{1}{T} \sum_{t=1}^T x_{t,M+1} \\
\subjectto \qquad & \frac{1}{T} \sum_{t=1}^T x_{t,i} \le B_i + \epsilon && \qquad \text{for all $i=1,2,\dots,M$} \\
& x_t \in \Psi(R_t,\epsilon) && \qquad \text{for all $t=1,2,\dots,T$}
\end{aligned}
\end{equation}
...
In the rest of the paper, we can thus assume that the requests are finite
non-empty subsets of $[-1,1]^{M+1}$. We denote by $\Requests_{\text{finite}}$
the set of all finite non-empty subsets of $[-1,1]^{M+1}$. We also assume that
a policy is a function from $\Requests_{\text{finite}}$ to $[-1,1]^{M+1}$.

We remark that similar reductions exists for the optimization problems
\eqref{equation:distribution-policy-optimization} and
...
$x_1, x_2, \dots, x_T$ be a sequence of actions that is $\epsilon'$-feasible and $\epsilon''$-optimal for
the optimization problem~\eqref{equation:optimization-problem-discretized}. Then,
$x_1, x_2, \dots, x_T$ is $(\epsilon + \epsilon')$-feasible and $(\epsilon + \epsilon'')$-optimal
...
\begin{align*}
\frac{1}{T} \sum_{t=1}^T x'_{t,M+1} & \ge V - \epsilon''' \: , \\
\frac{1}{T} \sum_{t=1}^T x'_{t,i} & \le B_i & \qquad \text{for all $i=1,2,\dots,M$,} \\
x'_t & \in R_t & \qquad \text{for all $t=1,2,\dots,T$.}
\end{align*}
In other words, $x'_1, x'_2, \dots, x'_T$ is feasible and $\epsilon'''$-optimal
for the optimization problem \eqref{equation:optimization-problem}.

We construct a sequence of actions $x''_1, x''_2, \dots, x''_T$. We define
...
$R_t$,
\begin{align*}
\|x''_t - x'_t\|_\infty \le \epsilon & \qquad \text{for all $t=1,2,\dots,T$.}
\end{align*}
Therefore,
\begin{align*}
\frac{1}{T} \sum_{t=1}^T x''_{t,M+1} & \ge V - \epsilon''' - \epsilon \: , \\
\frac{1}{T} \sum_{t=1}^T x''_{t,i} & \le B_i + \epsilon & \qquad \text{for all $i=1,2,\dots,M$,} \\
x_t'' & \in \Psi(R_t,\epsilon) & \qquad \text{for all $t=1,2,\dots,T$.}
\end{align*}
In other words, $x''_1, x''_2, \dots, x''_T$ is a feasible and $(\epsilon''' +
...
By assumption $x_1, x_2, \dots, x_T$ is $\epsilon'$-feasible and $\epsilon''$-optimal for
optimization problem \eqref{equation:optimization-problem-discretized}. This implies that
\begin{align*}
\frac{1}{T} \sum_{t=1}^T x'_{t,M+1} & \ge V - \epsilon - \epsilon'' \: , \\
\frac{1}{T} \sum_{t=1}^T x'_{t,i} & \le B_i + \epsilon + \epsilon' & \qquad \text{for all $i=1,2,\dots,M$,} \\
x_t' & \in \Psi(R_t,\epsilon) & \qquad \text{for all $t=1,2,\dots,T$.}
\end{align*}
Since $\Psi(R_t,\epsilon) \subseteq R_t$, this means that $x_1, x_2, \dots, x_T$
...
$\pi:\Requests_{\text{finite}} \to [-1,1]^{M+1}$ as follows. For any finite
