...
That is, if $x = (x_0, x_1, \dots, x_M) \in \R^{M+1}$, then
$$
\|x\|_p =
\begin{cases}
\left(|x_0|^p + |x_1|^p + \dots + |x_M|^p\right)^{1/p} & \text{if $p \in [1, \infty)$,}  \\
\max \{|x_0|, |x_1|, \dots, |x|_\infty\} & \text{if $p = \infty$.}
\end{cases}
$$
...
$\epsilon$-nets with respect to $\|\cdot\|_\infty$ norm. Formally, if $R
\subseteq [-1,1]^{M+1}$, an \emph{$\epsilon$-net} of $R$ is a subset $R'
\subseteq R$ such that for every $x \in R$, there exists $x' \in R'$ such that
$\|x - x'\|_\infty \le \epsilon$. The size of $\epsilon$-net of $R$ can be upper
bounded by $\left\lceil \frac{2}{\epsilon} \right\rceil^{M+1}$; see
Proposition~\ref{proposition:size-of-epsilon-net}. That size of the
$\epsilon$-net will become important later. We assume that we have an oracle
that given a subset $R \subseteq [-1,1]^{M+1}$ and $\epsilon > 0$ finds an
$\epsilon$-net of $R$ of size at most $\left\lceil \frac{2}{\epsilon}
\right\rceil^{M+1}$. We denote by $\Psi(R,\epsilon)$ the $\epsilon$-net that the
oracle outputs for input $R$ and $\epsilon$.
