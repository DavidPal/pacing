...
maximize a certain business metric (e.g. the total number clicks) subject to a
constraints on different business metrics (e.g. total advertisement cost, or
...
usually forecasted (e.g. by a time-series model). However, to keep things
...
we do assume that $T$ is known. Generalization of the formulation to unknown $T$
can be achieved by the doubling trick common on online learning.
...
approximately correct (PAC) model in learning theory~\cite{}. For packing
problems, the i.i.d. setting has been studied by~\cite{}. On the other hand, we
make very few assumptions on the sets $R_1, R_2, \dots, R_T$. We do allow each
$R_t$ to be finite or infinite, convex or non-convex. The only assumption we
make is that the requests are subsets of a bounded set. For concreteness, we
assume the requests are subsets of $[-1,1]^{M+1}$. However, any other bounded
set can be accomodated by scaling the requests.
