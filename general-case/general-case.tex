...
\begin{align}
\maximize_{a_1, a_2, \dots, a_T} \qquad & \frac{1}{T} \sum_{t=1}^T a_{t,M+1} \label{equation:optimization-problem-objective} \\
...
& a_t \in R_t && \qquad \text{for all $t=1,2,\dots,T$} \label{equation:optimization-problem-hard-constraints}
\end{align}
\end{subequations}
where $b_1, b_2, \dots, b_M$ are real numbers known ahead of time. The goal of
the algorithm is to choose a sequence of actions such that constraints
\eqref{equation:optimization-problem-hard-constraints} are satisfied,
constraints \eqref{equation:optimization-problem-constraints} are either
satisfied or violated by at most a small amount, and the objective
\eqref{equation:optimization-problem-objective} is within close to the optimal
value; see Section~\ref{subsection:approximate-solutions} for a formal
definition.
...
\eqref{equation:optimization-problem-constraints} and $b_i$ is known ahead of
time. As it will turn out knowing $T$ ahead of time is not neccesary in our
formulation. However, for simplicity we do assume that $T$ is known. Adapting
algorithms to unknown $T$ can be achieved by the standard doubling trick common
on online learning~\cite[Section 2.3.1]{Shalev-Shwartz-2012}. Third, in
practice, the optimization problem might be infeasible. In order to keep our
formulation simple, we focus on the case when the problem is feasible. However,
in practice, it is equally important to implement an algorithm that reports if
the problem is infeasible (or approximately infeasible) and either removes or
adjusts some of the constraints, or stops playing.
...
that a sequence of actions $a_1, a_2, \dots, a_T$ is \emph{$\epsilon$-optimal}
for the optimization problem \eqref{equation:optimization-problem} if
$$
...
We consider a simple example where $M=2$ and $b_1 = b_2 = 0$. For simplicity,
assume that $T$ is even. We construct the sequence of requests adversarially.
The first $T/2$ requests are
$$
R_1 = R_2 = \dots = R_{T/2} = \{ (1,-1,0), (-1,1,0) \}
$$
Depending on what actions the algorithm chooses in the first $T/2$ rounds, we
choose the remaining $T/2$ requests. We compute how many times each action
$(1,-1,0)$ and $(-1,1,0)$ was chosen by the algorithm in the first $T/2$ rounds.
Let $a_{\text{majority}}$ be the action with the greatest count, breaking ties
arbitrarily. The remaining $T/2$ requests are
$$
R_{T/2 + 1} = R_{T/2 + 2} = \dots = R_T = \{ a_{\text{majority}} \} \: .
$$

Since the last coordinate in all requests is zero, the
objective function of the optimization problem is zero for any sequence of
actions that satisfy
constraints~\eqref{equation:optimization-problem-hard-constraints}.
The optimal solution in hindsight depends on what the adversary chooses for the
second half of the requests. If $R_{T/2 + 1} = R_{T/2 + 2} = \dots = R_T = \{
(1,-1,0) \}$, then the optimal sequence of actions is
$$
a_1 = a_2 = \dots = a_{T/2} = (-1,1,0) \qquad \text{and} \qquad a_{T/2+1} = a_{T/2+2} = \dots = a_T = (1,-1,0) \: .
$$
On the other hand, if $R_{T/2 + 1} = R_{T/2 + 2} = \dots = R_T = \{ (-1,1,0) \}$, then the optimal
sequence of actions is
$$
a_1 = a_2 = \dots = a_{T/2} = (1,-1,0) \qquad \text{and} \qquad a_{T/2+1} = a_{T/2+2} = \dots = a_T = (-1,1,0) \: .
$$
The hind-sight solution satisfy constraints \eqref{equation:optimization-problem-constraints}.

However, any algorithm will come up
