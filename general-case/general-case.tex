...
\begin{abstract}
Many revenue management and resource allocation problems can be formulated as
an oline decision problem where an algorithm processes requests one by one. Each
request comes with a set of possible actions, and the algorithm needs to choose
irrevocably a single action for each request. Each action is associated with a
numerical vector consisting of the reward and one or more other metrics. The
goal of the algorithm is to maximize the average reward subject to constraints
on average of the other metrics. We study this problem under the assumption that
requests (and their action sets) are drawn i.i.d. from an unknown and arbitrary
probability distribution.

Previous works considered only packing problems where the metrics are
non-negative and the constraints cannot be violated. We allow the metrics to
have an arbitrary sign and we design an algorithm that finds a solution that is
approximately feasible and approximately optimal. More importantly, we do not
assume any structure on the set of actions, rewards or metrics.
\end{abstract}

\section{Introduction}

Revenue management and resource allocation problems can be often formulated as
optimization problems where the variables encode the actions that need to be
taken. However, often the actions need to be taken one by one, before the whole
optimization problem is revealed.

An example of such problem is online advertising where requests to show an
advertisement arrive as users visit a website. For each request an advertiser
needs to choose a bid in an auction. Depending on the contract the advertiser
has with the website, the advertiser pays the website either for each ad shown,
each time the user clicks on the ad, each time the user makes a purchase of the
advertised product, or some other action taken by the user. The price paid is
computed by the auction from the bid submitted by the advertiser and the bids
submitted by other competing advertisers. The goal of the advertiser is to
maximize a certain business metric (e.g. the total number of clicks) subject to
a constraints on other business metrics (e.g. total advertisement cost, or
revenue attributed to the advertisements) over a certain time period (e.g. a
month). For each possible choice of the bid, the business metrics, such as
probability of a click and expected cost for the ad, are predicted based on
user's features at the time when request is made. Based on these predictions the
advertiser needs to chose the bid for the request.

Another example of the same type of problem is ride hailing where the ride
hailing platform processes ride request from the users. For each ride request,
the platform needs to choose the price to show to the rider and a separate price
to show to the driver(s). The rider and the driver might choose or not to accept
the ride for the prices given. The goal of the ride hailing platform is to
maximize certain business metric (e.g. total number of rides taken) subject to
constraints on other business metrics (e.g. revenue, profit) over a certain time
period. The business metrics, such as probability of rider and driver accepting
the ride and expected revenue and profit, are predicted at the time when the
request is made based on the location of the rider and the locations of nearby
drivers, desired destination, length of the ride and time of the day. Based on
all this information the ride hailing platform chooses the price of the ride
request.

These problems can be formalized by the following online optimization problem.
An algorithm processes a sequence of \emph{requests} $R_1, R_2, \dots, R_T$ one
by one. Each request is a non-empty subset of $\R^{M+1}$, finite or infinite.
The elements of a request are called \emph{actions}. In round $t$, the algorithm
observes $R_t$ and it needs to choose an \emph{action} $a_t \in R_t$. The goal
of the algorithm is to solve approximately the optimization problem
\begin{subequations}
\label{equation:optimization-problem}
\begin{align}
\maximize_{a_1 \in R_1, a_2 \in R_2, \dots, a_T \in R_T} \qquad & \frac{1}{T} \sum_{t=1}^T a_{t,M+1} \label{equation:optimization-problem-objective} \\
\subjectto \qquad & \frac{1}{T} \sum_{t=1}^T a_{t,i} \le b_i && \qquad \text{for all $i=1,2,\dots,M$} \label{equation:optimization-problem-constraints}
\end{align}
\end{subequations}
where $b_1, b_2, \dots, b_M$ are real numbers known ahead of time. The goal of
the algorithm is to choose a sequence of actions such that the constraints
\eqref{equation:optimization-problem-constraints} are either satisfied or
violated by at most a small amount, and the objective
\eqref{equation:optimization-problem-objective} is close to the optimal value;
see Section~\ref{subsection:approximate-solutions} for a formal definition.

Of course, our formalization does not capture all the aspects of the
applications mentioned above. First, in the applications mentioned above, the
elements of $R_t$ are predicted by machine learning models just before the
action $a_t$ needs to be chosen. These predictions are only proxies for the
business metrics. The actual business metrics are revealed only after the action
has been chosen, sometimes with a significant delay. For example, in
the online advertising example, the user might click on the advertisement and
then purchase the product an hour later. However, in order to keep our problem
formulation simple, we ignore the difference between the predicted and the
actual business metrics. Second, in the applications mentioned, the constraints
are usually expressed as upper bounds on the total sum,
$$
\sum_{t=1}^T a_{t,i} \le B_i \qquad \text{for all $i=1,2,\dots,M$} \: ,
$$
instead of upper bounds on the average. If $T$ is known, translating between
$B_i$ and $b_i$ is trivial. However, in practice $T$ is unknown and it is
usually forecasted (e.g. by a time-series model). The forecasted value can be
used to translate between $B_i$ and $b_i$. However, to keep our formulation
simple, we assume that the constraints are of the form
\eqref{equation:optimization-problem-constraints} and both $b_i$ and $T$ are
known ahead of time. However, knowledge of $T$ ahead of time is not neccesary.
Our algorithms can be easily adapted if $T$ is unknown using the standard
doubling trick common in online learning literature~\cite[Section
2.3.1]{Shalev-Shwartz-2012}. Third, in practice, the optimization problem might
be infeasible. In order to keep our formulation simple, we focus on the case
when the problem is feasible. However, in practice, it is equally important to
implement an algorithm that reports if the problem is infeasible (or
approximately infeasible) and either removes or adjusts some of the constraints,
or stops playing.

Without any assumptions on the sequence of requests solving the online
optimization problem is information-theoretically impossible. We give a simple
proof of this in Section~\ref{section:impossibility-result}. In order to avoid
the impossibility result, we assume that sequence of requests is generated
i.i.d. from an arbitrary unknown distribution. This assumption is commonly used
in machine learning. For example, the i.i.d. assumption is the core assumption
of the probably approximately correct (PAC) model in learning
theory~\cite[Chapter 3]{Shalev-Shwartz-Ben-David-2014}. The i.i.d. assumption
was used for packing problems~\cite{Balseiro-Lu-Mirrokni-2023}.

On the other hand, we make very few assumptions on any indvidual set $R_t$. We
do allow each $R_t$ to be finite or infinite, convex or non-convex. The only
assumption we make is that the requests are subsets of the same bounded set. For
concreteness, we assume the requests are subsets of $[-1,1]^{M+1}$. However, any
other bounded set can be accomodated by scaling and/or shifting the requests.
This boundedness assumption has a natural interpretation that each requests has
only negligible impact on the average reward and averages of other metrics.

Finally, we assume that we have access to an oracle that given a request $R_t
\subseteq [-1, 1]^{M+1}$ constructs its $\epsilon$-net with respect to
$\|\cdot\|_{\infty}$ norm of size of at most $(C/\epsilon)^{O(M)}$ where $C$ is
some constant. In applications this assumption is easily satisfied.
...
\label{subsection:approximate-solutions}

We allow the online algorithm to solve the optimization problem
\eqref{equation:optimization-problem} within certain margin of error. For any
$\epsilon \ge 0$, we define the notion of $\epsilon$-feasible and
$\epsilon$-optimal solutions.

We say that a sequence of actions $a_1 \in R_1, a_2 \in R_2, \dots, a_T \in R_T$ is
...
$$
If $\epsilon=0$, an $\epsilon$-feasible solution is called simply feasible.

We define the \emph{optimal value} of the optimization problem
\eqref{equation:optimization-problem} as
$$
V = \sup \left\{\frac{1}{T} \sum_{t=1}^T a_{t,M+1} ~:~ a_1 \in R_1, \dots, a_T \in R_T, \ \forall i=1,2,\dots,M \quad \frac{1}{T} \sum_{t=1}^T a_{t,i} \le b_i  \right\} \: .
$$
Note that the optimal value might not be achieved by any feasible solution,
since the sets $R_1, R_2, \dots, R_T$ can be infinite and not closed. We say
that a sequence of actions $a_1 \in R_1, a_2 \in R_2, \dots, a_T \in R_T$ is
...
\eqref{equation:optimization-problem}.
...
We design an algorithm that finds solution $a_1, a_2, \dots, a_N$ of the
optimization problem \eqref{equation:optimization-problem} an approximately
feasible and approximately optimal. With probility at least $1 - \delta$, the
solution be $\epsilon$-feasible and $\epsilon$-optimal where $\epsilon =
\widetilde{O}(\sqrt{M \ln(1/\delta)/T})$.

We design of algorithm by a series of reductions. Each reduction introduces a
small error. First, we reduce the original problem to a special case where the
requests are finite and have cardinality at most $(C/\epsilon)^{O(M)}$. The
reduction relies on the oracle that constructs an $\epsilon$-net for each
request. Second, we reduce the problem to a special case where the probability
distribution has a density. In order to do that we add random noise to
coordinates of each action of each request. As long as the noise distribution
has a density, the distribution of the noisy requests has density as well.
Third, we reduce the optimization problem to a problem of finding an optimal
policy that maps requests to actions. We then prove that a linear multiclass
classifier is an optimal policy. The parameters of the linear classifier can be
learned from past requests. We use tools from statistical learning theory to
prove the necessary generalization bounds.

\subsection{Notation and definitions}
\label{subsection:notation-and-definitions}
...
\section{$\epsilon$-net reduction}
\label{section:epsilon-net-reduction}

We reduce the online optimization problem to a special case where the requests
are finite and of bounded cardinality. We replace each request by an
$\epsilon$-net. Formally, an \emph{$\epsilon$-net} of a set $R \subseteq
\R^{M+1}$ with respect to a norm $\|\cdot\|$ is a subset $R' \subseteq R$ such
that for every $x \in R$, there exists $x' \in R'$ such that $\|x - x'\| \le
\epsilon$. For the rest of paper, all $\epsilon$-nets are with respect
$\|\cdot\|_\infty$-norm.

The cardinality of the $\epsilon$-net is important. Any set $R \subseteq
[-1,1]^{M+1}$ has an $\epsilon$-net of size at most $\left\lceil
\frac{2}{\epsilon} \right\rceil^{M+1}$. Formal statement and proof can be found
in Appendix~\ref{section:size-of-epsilon-net} as
Proposition~\ref{proposition:size-of-epsilon-net}. We will use this bound in
later sections.

We assume that we have an oracle that given any subset $R \subseteq
[-1,1]^{M+1}$ and $\epsilon > 0$ finds an $\epsilon$-net of $R$ of size at most
$\left\lceil \frac{2}{\epsilon} \right\rceil^{M+1}$. Let $R_1, R_2, \dots, R_T
\subseteq [-1,1]^{M+1}$ be a sequence of requests. For $t=1,2,\dots,T$, let
$R_t'$ be an $\epsilon$-net. We reduce the online optimization problem
\eqref{equation:optimization-problem} to the online optimization problem
\begin{equation}
\label{equation:optimization-problem-discretized}
\begin{aligned}
\maximize_{a_1 \in R_1', a_2 \in R_2', \dots, a_T \in R_T'} \qquad & \frac{1}{T} \sum_{t=1}^T a_{t,M+1} \\
...
replaced by $R_t'$ and $b_i$ is replaced by $b_i + \epsilon$. The problem is
special in the sense that the requests $R_t'$ have finite cardinality, and, in
particular, the cardinality of each request is bounded by $\left\lceil
\frac{2}{\epsilon} \right\rceil^{M+1}$.
...
number. Then, there exists a sequence of actions $a_1' \in R_1, a_2' \in R_2, \dots, a_T' \in R_T$ such that
\begin{align*}
...
We construct a sequence of actions $a_1'' \in R_1', a_2'' \in R_2', \dots, a_T'' \in R_T'$. We define
$a_t''$ be the element of $R_t'$ that is closest to $a_t'$ in the
$\|\cdot\|_\infty$-norm. Since $R_t'$ is an $\epsilon$-net of $R_t$,
\begin{align*}
\|a_t'' - a_t'\|_\infty \le \epsilon & \qquad \text{for all $t=1,2,\dots,T$.}
\end{align*}
Therefore,
\begin{align*}
...
By assumption $a_1 \in R_1', a_2 \in R_2', \dots, a_T \in R_T'$ is $\epsilon'$-feasible and $\epsilon''$-optimal for
optimization problem \eqref{equation:optimization-problem-discretized}. This implies that
\begin{align*}
\frac{1}{T} \sum_{t=1}^T a_{t,M+1} & \ge V - \epsilon - \epsilon'' \: , \\
\frac{1}{T} \sum_{t=1}^T a_{t,i} & \le b_i + \epsilon + \epsilon' & \qquad \text{for all $i=1,2,\dots,M$.}
\end{align*}
Since $R_t' \subseteq R_t$, this means that $a_1, a_2, \dots, a_T$
...
suppose $R \sim D$. Let
$$
Z = ((Z_{1,1}, Z_{1,2}, \dots, Z_{1,M+1}), \dots, (Z_{N,1}, Z_{N,2}, \dots, Z_{N,M+1}))
$$
be an $N$-tuple of vectors in $\R^{M+1}$ where the entrie $Z_{1,1}, \dots,
Z_{N,M+1}$ are i.i.d. random variables drawn from the uniform distribution over
...
\begin{theorem}[Impossibility of feasibility]
Let $A$ be any online algorithm, possibly randomized. Let $T \ge 4$ be an
integer. Let $M=2$ and $b_1 = b_2 = 0$. There exists a sequence of requests
$R_1, R_2, \dots, R_T \subseteq [-1,1]^{M+1}$ such that the optimization
problem~\eqref{equation:optimization-problem} is feasible, and with probability
at least $1/2$, the solution constructed by $A$ is not $\frac{1}{3}$-feasible.
\end{theorem}

\begin{proof}
We construct the sequence of requests by an adversarial argument. The first
$\lfloor T/2 \rfloor$ requests are
$$
R_1 = R_2 = \dots = R_{\lfloor T/2 \rfloor} = \{ (1,-1,0), (-1,1,0) \} \: .
$$
Let $a_1, a_2, \dots, a_{\lfloor T/2 \rfloor}$ be the actions chosen by the
algorithm $A$ in the first $\lfloor T/2 \rfloor$ rounds. Let $n_1, n_2$ be
the number of times each of the two action $(1,-1,0)$, $(-1,1,0)$ gets chosen by
the algorithm in the first $\lfloor T/2 \rfloor$ rounds. Formally,
\begin{align*}
n_1 & = \{ t ~:~ 1 \le t \le \lfloor T/2 \rfloor, a_t = (1,-1,0) \} \\
n_2 & = \{ t ~:~ 1 \le t \le \lfloor T/2 \rfloor, a_t = (-1,1,0) \}
\end{align*}
Since the algorithm $A$ can be randomized, $n_1$ and $n_2$ are random variables.
Either $n_1 \ge n_2$ with probability at least $1/2$, or $n_2 \le n_1$ with
probability at least $1/2$. Let $i^* \in \{1,2\}$ be such that $n_{i^*} \ge n_{3 - i^*}$
with probability at least $1/2$. Accordingly, we define the majority action
chosen by the algorithm as
$$
a^* =
\begin{cases}
(1,-1,0) & \text{if $i^*=1$,} \\
...
\end{cases}
$$
We define the last $\lfloor T/2 \rfloor$ requests as
$$
R_{\lceil T/2 \rceil + 1} = R_{\lceil T/2 \rceil + 2} = \dots = R_T = \{ a_{\text{majority}} \} \: .
$$
If $T$ is odd, we define $R_{\lceil T/2 \rceil} = \{ (0, 0, 0)\}$. If $T$ is even, then
$R_{\lceil T/2 \rceil} = R_{\lfloor T/2 \rfloor}$ was already defined.

The optimization problem~\eqref{equation:optimization-problem} for this sequence
of requests has only one feasible solution $\tilde{a}_1, \tilde{a}_2, \dots,
\tilde{a}_T$,
\begin{align*}
\tilde{a}_1 & = \tilde{a}_2 = \dots = \tilde{a}_{\lfloor T/2 \rfloor} = - a^* \\
\tilde{a}_{\lceil T/2 \rceil + 1} & = \tilde{a}_{\lceil T/2 \rceil + 2} = \dots = \tilde{a}_T = a^* \\
\tilde{a}_{\lceil T/2 \rceil} & = (0, 0, 0) & \text{if $T$ is odd}
\end{align*}

Consider the solution $a_1, a_2, \dots, a_T$ produced by the online algorithm
$A$. Consider the $i^*$-th constraint
\eqref{equation:optimization-problem-constraints}. The left-hand side of the
constraint is at least
$$
\frac{1}{T} \sum_{t=1}^T a_{t,i^*} = \frac{1}{T} \left( n_{i^*} - n_{3 - i^*} + \lfloor T/2 \rfloor \right) \ge \frac{\lfloor T/2 \rfloor}{T} \ge \frac{1}{2} - \frac{1}{2T} \: .
$$
For $T > 3$, the left-hand side is strictly more than $1/3$. Since the
right-hand side of the constraint is $b_{i^*} = 0$, the solution is not
$\frac{1}{3}$-feasible.

We remark that that the objective value of any solution, feasible or not, is $0$.
This includes the optimal solution and the solution produced by the algorithm $A$.
\end{proof}
