...
needs to choose a bid in an auction. Depending on the contract with the website,
the advertiser pays the website either for each ad shown, each time the user
clicks on the ad, each time the user makes a purchase of the advertised product,
or some other action taken by the user. The price paid for the request is
computed by the auction from the bids submitted by the advertisers. The goal of
the advertiser is to maximize a certain business metric (e.g. the total number
clicks) subject to a constraints on different business metrics (e.g. total
advertisement cost or revenue attributed to the advertisements) over a certain
time period (e.g. a month). For each possible choice of the bid, the business
metrics, such as probability of a click and expected cost for the ad, are
predicted based on user's features at the time when request is made. Based on
these predictions the advertiser needs to chose the bid for the request.

Another example is ride hailing where the ride hailing platform
processes ride request from the users. For each ride request, the platform needs
to choose the price to show to the rider and a separate price to show to the
driver(s). The rider and the driver might choose or not to accept the ride for
the prices given. The goal of the ride hailing platform is to maximize certain
business metric (e.g. total number of rides taken) subject to constraints on
other business metrics (e.g. revenue, profit) over a certain time period. The
business metrics, such as probability of rider and driver accepting the ride and
expected revenue and profit, are predicted at the time when the request is made
based on the location of the rider and the locations of nearby drivers, length
of the ride and time of the day. Based on all this information the ride hailing
platform chooses the price of the ride request.

Without any assumptions on the sequence of requests solving the optimization
problem is information-theoretically impossible. We study the problem in the
i.i.d. model;~\cite{Balseiro-Lu-Mirrokni-2023}. We disregard any assumptions on
the action sets and their associated rewards. In particular, we allow to the
action sets to be infinite and non-convex.
