...
way we can make have requests to be tuples of the same length. If $R = (a_1,
a_2, \dots, a_N)$ is an $N$-tuple, the set notation $a \in R$ means that $a =
a_i$ for some $i = 1,2,\dots,N$. If all requests are tuples of the same length
$N$, a policy is function $\pi: [-1,1]^{(M+1) \times N} \to [-1,1]^{M+1}$ such
that $\pi(R) \in R$.
...
As explained in Section~\ref{section:discretization}, we assume that the
...
with respect to the Lebesgue measure on $\R^{N \times (M+1)}$. The fundamental
...
small independent noise to each entry of the request matrix. Namely, suppose $R
\sim D$. Let $Z$ be an independent random $N \times (M+1)$ matrix with entries
drawn i.i.d. from the uniform distribution over $[-1,1]$. We construct a random
sample $R'$ from $D'$ as
\begin{equation}
...
\begin{aligned}
\maximize_{\pi' \in \Pi} \qquad & \Exp_{R' \sim D'} \left[ \sum_{i=1}^N \pi'(R')_i \cdot R'_{i,0} \right] \\
\subjectto \qquad & \Exp_{R' \sim D'} \left[ \sum_{i=1}^N \pi'(R')_i \cdot R'_{i,j} \right] \le B_j' & \text{for all $j=1,2,\dots,M$}
\end{aligned}
\end{equation}
where
$$
