...
In the online marketing and ride hailing applications, the requests change over
time; the sequence of requests has seasonal patterns, often at multiple time
scales. Nevertheless, the i.i.d. assumption is a good approximation for short
enough time periods (e.g., one hour, one day, or one week).
...
Under three assumptions, we design an algorithm that solves the online
optimization problem approximately. The idea behind the algorithm is find an
approximately optimal policy. A \emph{policy} $\pi$ is a function
$\pi:2^{\R^{M+1}} \to \R^{M+1}$ such that $\pi(R) \in R$ for all $R \subseteq
\R^{M+1}$. A policy chooses an action $\pi(R)$ for any request $R$. A randomized
policy is a distribution over policies. We denote by $\P$ the set of all
randomized policies. The algorithm's goal is to find a randomized policy $P \in
\P$ that is an approximate solution of the optimization problem
\begin{equation}
...
\pi_\lambda(R) = \argmax_{x \in R_t} \lambda_1 R_{i,1} + \lambda_2 R_{i,2} + \dots + \lambda_M R_{i,M} + \lambda_{M+1} R_{i,M+1} \: .
\end{equation}
