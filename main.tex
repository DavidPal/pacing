...
Each request is an $N \times (M+1)$ matrix with entries in the interval $[-1,1]$. In
other words, we assume that the space of requests is $\Requests = [-1,1]^{N
...
\end{align*}
where the maximization is over the set $\Pi$ of all set of measurable
policies.
...
Condition on the event $R_1=r_1, R_2=r_2, \dots, R_T=r_T$ and consider the set of vectors
$$
...
$$
This is a finite set of $T$-dimensional vectors. We see that
$$
\Exp\left[\sup_{\substack{\lambda \in \R^{M+1} \\ j=0,1,2,\dots,M}} \left| \sum_{t=1}^T \sum_{i=1}^N \sigma_t \cdot \pi_\lambda(R_t)_i \cdot R_{t,i,j}  \right| ~\bigg|~ R_1=r_1, R_2=r_2, \dots, R_T=r_T \right]
...
The output of a policy $\pi_\lambda$ on a request $r_t$ is determined by $\binom{N}{2}$ indicators
$$
