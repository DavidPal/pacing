...
\dots, \lambda_M$ that are not all zero. As will become clear later, the
$\lambda_1, \dots, \lambda_M$ can be interpreted as the dual variables
associated with the constraints of the linear program. However, for now, think
of them as arbitrary real numbers. For any $(M+1)$-tuple $\lambda = (\lambda_0,
\lambda_1, \dots, \lambda_M)$ we define a policy $\pi_\lambda:\Requests \to
\Delta_N$ as follows. For any request $R \in \Requests$, we define
$$
\pi_\lambda(R) = e_{i^*}
...
loss of generality that $\|\lambda\|_2 = 1$. We denote by
$\Pi_\Lambda^\text{det}$ the set of deterministic Lagrangian policies. Formally,
that is,
$$
\Pi_\Lambda^\text{det} = \left\{\pi_\lambda ~:~ \lambda \in \R^{M+1}, \|\lambda\|_2 = 1 \right\} \: .
$$
...
$\sigma$-sub-Gaussian random variable with $\sigma=\sqrt{T}$. That is, $Y_v$ satisfies
$$
...
dimension of homogeneous hyperplanes in $\R^{M+1}$ is $M+1$. Thus, the number of
