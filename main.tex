...
adding a small independent noise to each entry $R_{t,i,j}$ of a request. Namely,
we replace $R_{t,i,j}$ with
$$
R'_{t,i,j} = (1-\epsilon)R_{t,i,j} + \epsilon Z_{t,i,j}
$$
where $\epsilon \in (0,1)$ is a small positive number and $Z_{t,i,j}$ is drawn
independently from the uniform distribution over $[-1,1]$. The value of
...
Suppose that the requests $R_1, R_2, \dots, R_T$ are in general
position. Let $\mu_1, \dots, \mu_T$, $\lambda_1, \dots, \lambda_M$ be an optimal
solution of the dual program. Let $x_{1,1}, \dots, x_{T,N}$ be any optimal
solution of the primal program. Let $\lambda_0 = 1$ and $lambda = (\lambda_0,
\lambda_1, \dots, \lambda_M)$ and let $\pi_\lambda$ corresponding Lagrangian
policy. Then, for every $j=0,1,\dots,M$,
$$
