...
``general position'', a term that we define precisely later. Finally, we show
that if the requests are sampled i.i.d. from a probability distribution with a
probability density then they are in general position with probability one. In
summary, the optimal solutions of both linear programs can be essentially
encoded using only $M$ parameters $\lambda_1, \dots, \lambda_M$.
...
Let $R_1, R_2, \dots, R_T \in \Requests$ be requests. The requests are in
...
\begin{lemma}[Few ties]
Suppose that the requests $R_1, R_2, \dots, R_T$ be requests in general
position. Let $\mu_1, \dots, \mu_T$, $\lambda_1, \dots, \lambda_M$ be an optimal
solution of the dual program. With probability one, for at least $T-M$ values of
the index $t$, the argmax
\begin{equation}
...
$$
In other words, $\lambda_1, \lambda_2, \dots, \lambda_M$ satisfy the $|\T|$ linear equations
$$
\sum_{j=1}^M (R_{t,i_t,j} - R_{t,i'_t,j}) \lambda_j = R_{t,i_t,0} - R_{t,i'_t,0} \: ;
$$
one equation for every $t \in \T$.

Suppose by contradiction that $|\T| \ge M + 1$. Let $\T' \subseteq \T$ be any
subset of size exactly $M+1$. Consider the system of linear equations
corresponding to $\T'$. Since the request are in general position, the $M+1$
equations in this system are linearly independent. However, since there are only
$M$ variables, this system of equations does not have any solution. This
contradicts that $\lambda_1, \lambda_2, \dots, \lambda_M$ satisfy all these
equations.
\end{proof}
